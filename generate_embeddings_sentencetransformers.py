"""
Sentence-Transformers Embeddings Generation Script
================================================

This script generates embeddings for text and labels using sentence-transformers with task-specific prompts.
Saves embeddings to an /embeddings directory for reuse in semantic similarity analysis.

Usage:
    uv run python generate_embeddings_sentencetransformers.py
"""
...
class SentenceTransformersEmbeddingsGenerator:
    def __init__(self, data_path: str, model: str = "all-mpnet-base-v2", output_dir: str = "embeddings"):
        """
        Initialize the embeddings generator using sentence-transformers.
        
        Parameters:
        data_path: str, path to the CSV file with data
        model: str, sentence-transformers model to use
        ...
        # Initialize sentence-transformers model with prompts
        self.model = SentenceTransformer(model)
        
        # Define task-specific prompts to improve embedding quality
        self.prompts = {
    ...
    def get_unique_texts_and_labels(self):
        """
        ...
        if 'label' in self.data.columns:
            ...
            results['human_labels'] = unique_labels
        ...
        if 'snafu_gt_label' in self.data.columns:
            ...
            results['snafu_labels'] = unique_snafu
        ...
        if 'labelLLM' in self.data.columns:
            ...
            results['llm_labels'] = unique_llm_labels
            print(f"Unique normalized LLM labels: {len(unique_llm_labels)}")
        
        return results
    
    def generate_embeddings_batch(self, texts: List[str], prompt: str, batch_size: int = 32) -> Dict[str, List[float]]:
        """
        Generate embeddings for a list of texts using batch processing with prompts.
        
        Parameters:
        texts: List of texts to embed
        prompt: Task-specific prompt to prepend to each text
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                # Prepend prompt to each text in the batch
                prompted_texts = [prompt + text for text in batch]
                
                # Generate embeddings
                batch_embeddings = self.model.encode(prompted_texts, convert_to_numpy=True)
                ...
                for text in batch:
                    try:
                        ...
                        embedding = self.model.encode([prompted_text], convert_to_numpy=True)[0]
                        embeddings[text] = embedding.tolist()
    ...
    def generate_all_embeddings(self):
        """
        Generate embeddings for all unique texts and labels with appropriate prompts.
        """
        print("=" * 60)
        ...
        unique_data = self.get_unique_texts_and_labels()
        
        # Generate embeddings for texts with word prompt
        print("\nGenerating text embeddings with word prompt...")
        self.text_embeddings = self.generate_embeddings_batch(
            unique_data['texts'], 
            self.prompts['word']
        )
        
        # Generate embeddings for human labels if available
        if 'human_labels' in unique_data:
            print("\nGenerating human label embeddings with label prompt...")
            self.label_embeddings = self.generate_embeddings_batch(
                unique_data['human_labels'], 
                self.prompts['label']
            )
        
        # Generate embeddings for SNAFU categories if available
        if 'snafu_labels' in unique_data:
            print("\nGenerating SNAFU category embeddings with category prompt...")
            self.snafu_embeddings = self.generate_embeddings_batch(
                unique_data['snafu_labels'], 
                self.prompts['snafu']
            )
        
        # Generate embeddings for LLM labels if available
        if 'llm_labels' in unique_data:
            print("\nGenerating LLM label embeddings with label prompt...")
            self.llm_embeddings = self.generate_embeddings_batch(
                unique_data['llm_labels'], 
                self.prompts['label']
            )
    ...
    def save_embeddings(self):
        ...
        # Create metadata
        embedding_dim = len(list(self.text_embeddings.values())[0]) if self.text_embeddings else 0
        metadata = {
        ...
        # Save combined embeddings file (compatible with existing pipeline)
        embeddings_data = {
        ...
        # Save combined file
        embeddings_path = os.path.join(self.output_dir, "sentencetransformers_embeddings.pkl")
        with open(embeddings_path, 'wb') as f:
            pickle.dump(embeddings_data, f)
        print(f"Combined embeddings saved to: {embeddings_path}")
        
        # Save metadata as JSON for easy inspection
        metadata_path = os.path.join(self.output_dir, "sentencetransformers_embeddings_metadata.json")
        with open(metadata_path, 'w') as f:
            json.dump(metadata, f, indent=2)
        print(f"Metadata saved to: {metadata_path}")
        
        # Save data mapping for analysis (including normalized versions)
        mapping_columns = ['text', 'text_normalized', 'playerID', 'category', 'word_index']
        
        # Add available label columns
        if 'label' in self.data.columns:
            mapping_columns.extend(['label', 'label_normalized'])
        if 'snafu_gt_label' in self.data.columns:
            mapping_columns.extend(['snafu_gt_label', 'snafu_gt_label_normalized'])
        if 'labelLLM' in self.data.columns:
            mapping_columns.extend(['labelLLM', 'labelLLM_normalized'])
        
        data_mapping = self.data[mapping_columns].copy()
        mapping_path = os.path.join(self.output_dir, "sentencetransformers_data_mapping.csv")
    ...
    def run_generation(self):
        ...
        """
        print("Starting Sentence-Transformers Embeddings Generation")
        print("=" * 60)
        ...
        print("\nSentence-Transformers embeddings generation completed successfully!")
        print(f"All outputs saved to: {self.output_dir}")

def find_csv_files():
    """Find all CSV files in the project directories (excluding virtual environment)."""
    csv_files = []
    search_dirs = ['data', 'process_for_labels', 'analysis_output', 'files_from_exp1', 'old_files', 'claude_orig_attempt']
    
    # Also look for CSV files in the current directory (not subdirectories)
    try:
...
def select_sentence_transformer_model():
...
def main():
    ...
    print("Sentence-Transformers Embeddings Generation")
    print("=" * 60)
    ...
    model = select_sentence_transformer_model()
    if model is None:
        return
    ...
    generator = SentenceTransformersEmbeddingsGenerator(data_path, model)
