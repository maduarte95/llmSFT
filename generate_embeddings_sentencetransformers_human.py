"""
Sentence-Transformers Human Embeddings Generation Script
=======================================================

This script generates embeddings for text, human labels, and SNAFU categories using sentence-transformers.
Saves embeddings to an /embeddings directory for reuse in semantic similarity analysis.

Usage:
    uv run python generate_embeddings_sentencetransformers_human.py
"""
...
class SentenceTransformersHumanEmbeddingsGenerator:
    def __init__(self, data_path: str, model: str = "all-mpnet-base-v2", output_dir: str = "embeddings"):
        """
        Initialize the embeddings generator for human labels using sentence-transformers.
    ...
    def generate_all_embeddings(self):
        ...
        """
        print("=" * 60)
        print("GENERATING HUMAN EMBEDDINGS WITH SENTENCE-TRANSFORMERS")
        print("=" * 60)
        print(f"Using model: {self.model_name}")
        
        unique_texts, unique_labels, unique_snafu = self.get_unique_texts_and_labels()
    ...
    def save_embeddings(self):
        ...
        # Save combined file
        embeddings_path = os.path.join(self.output_dir, "sentencetransformers_human_embeddings.pkl")
        with open(embeddings_path, 'wb') as f:
            pickle.dump(embeddings_data, f)
        print(f"Human embeddings saved to: {embeddings_path}")
        
        # Save metadata as JSON for easy inspection
        metadata_path = os.path.join(self.output_dir, "sentencetransformers_human_embeddings_metadata.json")
        with open(metadata_path, 'w') as f:
            json.dump(metadata, f, indent=2)
        print(f"Metadata saved to: {metadata_path}")
        
        # Save data mapping for analysis (including normalized versions)
        # Use standard naming convention for compatibility with analysis scripts
    ...
    def run_generation(self):
        ...
        """
        print("Starting Sentence-Transformers Human Embeddings Generation")
        print("=" * 60)
        ...
        print("\nSentence-Transformers human embeddings generation completed successfully!")
...
def main():
    ...
    print("Sentence-Transformers Human Embeddings Generation")
    print("=" * 60)
    ...
    generator = SentenceTransformersHumanEmbeddingsGenerator(data_path, model)
